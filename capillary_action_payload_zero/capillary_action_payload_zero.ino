...
void loop(){
  //rtc
   ...
   uint8_t time[6] = {rtc.getYear(), rtc.getMonth(), rtc.getDay(), rtc.getHours(), rtc.getMinutes(), rtc.getSeconds()};
  pictures(); 

  

//convert analog input to m/s^2: (x - 512) * (.05753)
/*  int analog_x = ReadAxis(accel_xInput);
  int analog_y = ReadAxis(accel_yInput);
  int analog_z = ReadAxis(accel_zInput); */
  float accel_x = (ReadAxis(accel_xInput)-512)*(.05753)+20;
  float accel_y = (ReadAxis(accel_yInput)-512)*(.05753)+20;
  float accel_z = (ReadAxis(accel_zInput)-512)*(.05753)+20;
...
//print to SD card solenoid status
  ...
  if (SD.exists("AccelDat.txt")) {
   ...
   // if(/*altitude > 7000 && */((accel_total > -1 && accel_total < 1) || (accel_total < 10.8 && accel_total > 8.8) || (accel_total > -10.8 && accel_total < -8.8))){ 
    if(((accel_total - accel_cal) > -11.8) && ((accel_total - accel_cal) < -7.8)){
